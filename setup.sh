...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/zeronxdev/xrayr/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Không xác định được phiên bản XrayR${plain}"
            exit 1
        fi
        echo -e "Phiên bản XrayR mới nhất: ${last_version}，Bắt đầu cài đặt"
        wget -q -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/zeronxdev/xrayr/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/zeronxdev/xrayr/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/zeronxdev/xrayr/main/xrayz.sh
